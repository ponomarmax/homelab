...
class VectorWindowFeatureBuilder:
    name = "VectorWindowFeatureBuilder"
    ...
    def handle(self, clean_df: pd.DataFrame, run_id: str, input_artifact_reference: str) -> FeatureHandlerOutput:
        ...
        for window_size, freq in WINDOWS:
            ...
            for window_start, group in grouped:
                ...
                z = pd.to_numeric(group["z"], errors="coerce")
                v = pd.to_numeric(group["vector_magnitude"], errors="coerce")
                ...
                rows.append(
                    {
                        ...
                        "max_y": float(y.max()),
                        "max_z": float(z.max()),
                        "vector_magnitude_mean": float(v.mean()),
                        "vector_magnitude_std": float(v.std(ddof=0)),
                        "vector_magnitude_max": float(v.max()),
